...
import hu.bme.mit.inf.dslreasoner.viatrasolver.reasoner.StochasticSimulation
...
class SolverLoader {
	def loadSolver(Solver solver, Map<String, String> config) {
		switch (solver) {
			...
			case VIATRA_SOLVER: return new StochasticSimulation
		}
	}
	...
	def loadSolverConfig(Solver solver, Map<String, String> config, List<ObjectiveEntry> objectiveEntries,
		...
		switch (solver) {
			...
			case VIATRA_SOLVER: {
				...
				if (config.containsKey("edgeNodeRatio")) {			//TODO try-catch
					val stringValue = config.get("edgeNodeRatio")
					c.edgeNodeRatio = Double.parseDouble(stringValue)
				}
				if (config.containsKey("maxSteps")) {			//TODO try-catch
					val stringValue = config.get("maxSteps")
					c.maxSteps = Integer.parseInt(stringValue)
				}
